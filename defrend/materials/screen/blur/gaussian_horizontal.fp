...
void main() {
	...
	for (int i = 0; i < 5; ++i) {
		...
		// sacrificing quality.
		vec2 xy_r = gl_FragCoord.xy + delta * i + 0.5;
		vec2 xy_l = gl_FragCoord.xy - delta * i - 0.5;
		result += texture(color_sampler, xy_r / resolution).rgb * weight[i];
		result += texture(color_sampler, xy_l / resolution).rgb * weight[i];
	}
